...
При изменении формата ключей Redis или добавлении другого механизма кеширования, основная логика HTTP endpoints останется без изменений.
...
```mermaid
flowchart LR
    Client["Client"] -->|HTTP REST| API["FastAPI application"]
...
**Redis** — кеш для оптимизации повторного чтения постов. Он не является главным источником данных.

Такой подход позволяет оптимизировать процесс чтения часто запрашиваемых постов и при этом не нагружать PostgreSQL, оставляя его как единственное хранилище актуального состояния.
...
Переменные окружения разделены по назначению: настройки приложения, подключения к внешним сервисам и параметры кеширования.

#### Настройки приложения

| Переменная | Назначение |
|---|---|
| `APP_NAME` | Техническое имя сервиса. Используется в healthcheck, логах и служебных ответах |
| `APP_TITLE` | Название приложения в OpenAPI/Swagger UI |
| `APP_DESCRIPTION` | Краткое описание сервиса |
| `APP_ENV` | Текущее окружение приложения |
| `APP_VERSION` | Версия приложения |
| `API_VERSION` | Версия публичного API |
...
| Переменная | Назначение |
|---|---|
| `DATABASE_URL` | Строка подключения к PostgreSQL через asyncpg |
| `REDIS_URL` | Строка подключения к Redis |

Для запуска через Docker Compose используются внутренние имена сервисов:
...
Здесь `postgres` и `redis` — это имена сервисов из `docker-compose.yml`, доступные внутри Docker-сети.

#### Настройки SQLAlchemy

| Переменная | Назначение |
|---|---|
| `SQLALCHEMY_ECHO` | Включает или отключает вывод SQL-запросов SQLAlchemy в логи |
...
Если установить `true`, SQLAlchemy начнет выводить SQL-запросы в логи. Это может быть полезно при отладке, но в обычном запуске лучше оставлять значение `false`.

#### Настройки кеширования

| Переменная | Назначение |
|---|---|
| `POST_CACHE_TTL_SECONDS` | Время жизни кешированной записи поста в Redis, в секундах |
...
Это означает, что запись о посте может храниться в Redis до 300 секунд.
...
При любом изменении значений в `.env`, уже запущенный контейнер может продолжить использовать старые переменные окружения.

Для того, чтобы Docker пересоздал контейнер API с новыми значениями, необходимо выполнить:
...
Проект полностью контейнеризирован и запускается из Docker.
...
Если PostgreSQL или Redis еще не успели стать `healthy`, необходимо подождать несколько секунд и повторить команду:
...
- **Системные проверки** — используются для проверки состояния сервиса;
- **Управления постами** — реализуют CRUD-операции и сценарии кеширования.
...
`Cache hit` возникает, когда Redis уже содержит пост по нужному ключу.

В этом случае приложение возвращает данные из Redis и не выполняет дополнительный запрос в PostgreSQL.

`Cache miss` возникает, когда нужной записи в Redis нет.
